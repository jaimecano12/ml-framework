...
\keynum{12} datasets (6+6) $\cdot$ \keynum{343} tests $\cdot$
...
today and why data leakage is such a critical problem. Second, the system
...
this rapid adoption has a cost nobody talks about enough: according to
Anaconda's State of Data Science survey, data scientists spend nearly forty
percent of their time on data preparation and cleaning --- more than they
spend building or training models. So while everyone races to adopt AI, the
data feeding these systems is too often the weakest link --- and that is
...
one stands out as the most insidious: data leakage --- information about the
target reaching the training features in a way that will not exist at
prediction time. What makes it
...
This thesis sets out to automate, quantify, and make actionable exactly that
kind of detection, in a single tool.}

\slide{6}{A Motivating Example}{1:15}{5:05}
{Pausa dramática tras decir "one point zero, zero, zero". Señala la flecha.}
{To quantify this impact, I wanted to build an example scenario. I trained a
logistic regression classifier on a dataset containing one engineered leaky
feature. Validation accuracy: one
...
{Closing that gap is exactly what this thesis sets out to do. This
translates into four contributions. One: a unified, automated framework
...
tools. Everything is validated on twelve datasets, covered by 343 unit tests,
...
{Now that you have seen how the pipeline works internally, let us look at how
you would actually use it. The same pipeline is exposed through three interfaces: a CLI for batch runs
...
{Here we can see the complete check catalog --- I won't read it all. Every
check works the same simple way: it passes or fails, and tells you exactly
which columns are affected. The leakage row is where the novelty lives, so
let's zoom into its fifth element: the unified risk score.}
...
that each of them can be fooled: correlation misses non-linear leaks, mutual
information alone is noisy, and both can be diluted by missing values. So for
every feature I combine three normalized signals. Rho just measures how
correlated a feature is with the target. I-tilde is mutual information,
normalized by the maximum across features. And pi is what I call performance
inflation: it trains a tiny model on just that one feature and checks how
close it gets to perfect accuracy. The weighted combination gives a score
between zero and one. At 0.7 the framework raises a
...
sensitivity analysis on real leaky features from the Titanic dataset, where
we are predicting passenger survival. The feature
...
Imagine an ICU mortality dataset, where we are predicting whether a patient
survives, with a feature called
...
model, which evaluates each feature and
returns a risk level, from none to high, a leakage type --- temporal, proxy, post-hoc or indirect
--- and a natural-language rationale explaining why. The module is optional,
...
{To check if this actually works, I tested it on thirty real feature names I
labelled by hand --- including cases like the \emph{discharge\_date} example
I just showed you --- and ran it live against Claude Haiku 4.5 through AWS
Bedrock. At the model's normal sensitivity setting, it correctly separates
every risky feature from every safe one, with zero mistakes. If I make it
stricter, so it only flags the clearest cases, it makes three small,
understandable errors --- for example, treating one genuinely ambiguous
feature as riskier than it really is. Because this is a real, independent
model, and not a script that already knew the answers, these mistakes
actually tell us something honest about how it reasons.}
...
planted deliberately, and six real-world datasets, all downloaded from
OpenML, the open machine-learning data repository built on the UCI Machine
Learning Repository --- ranging from a few hundred rows up to forty-eight
thousand. The synthetic sets verify the
...
{Titanic is a dataset of 1,309 passengers, where the target is whether each
one survived --- and it is also the most famous teaching dataset in the
world. So why does it get a B? The framework found two real leaks. First, \emph{name}: with 1,300 unique
...
fourteen modules, and 343 tests, all passing.}
