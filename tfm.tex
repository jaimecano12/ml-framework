...
\label{tab:phases}\\
\toprule
\textbf{Phase} & \textbf{Name} & \textbf{Key artefact(s)} & \textbf{Tests} \\
\midrule
\endfirsthead
\multicolumn{4}{c}{\tablename\ \thetable\ --- continued}\\
\toprule
...
\toprule
\textbf{Dataset} & \textbf{Type} & \textbf{Rows} & \textbf{Primary purpose / defect profile} \\
\midrule
...
These eight contributions, taken together, satisfy the seven objectives set out in
Chapter~\ref{chap:intro}.  In particular, the unified leakage risk score
$\mathcal{L}(f)\in[0,1]$ (Equation~\eqref{eq:lrs}) addresses the long-standing limitation of
single-signal heuristics by combining correlation, mutual information, and performance
inflation into one interpretable number; the GPT-4o-mini semantic module extends detection
to feature-naming patterns that no statistical signal can see; and the impact-analysis
module turns an abstract leakage flag into a concrete, measurable consequence---removing the
leaky feature from \texttt{leaky\_dataset} reduces accuracy by $\Delta{=}{-}0.048$, direct
evidence that the original model was exploiting leaked information rather than learning a
...
\texttt{dirty\_dataset}) through grade B (72--82/100, \texttt{leaky\_dataset}, Titanic, Pima
Diabetes, Wine Quality) to grade A (92.4--97.5/100, Adult Census, German Credit; 95/100,
\texttt{clean\_dataset}).  Real-world case studies on Titanic, Adult Census, and German
Credit (Section~\ref{sec:casestudies}) confirm that the framework correctly identifies
domain-specific problems, including the well-known \texttt{boat} leakage in Titanic and
near-irrelevant features in German Credit.  The quantitative benchmark of
Section~\ref{sec:benchmark} shows that ml-framework covers 29/29 ML-relevant checks and
detects 4/4 constructed leakage scenarios, while the best competing tool (Deepchecks)
covers 11/29 checks and detects at most 1/4 scenarios.
